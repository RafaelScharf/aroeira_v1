...
class Auth extends MY_Controller
{
    ...
    function change_password()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $change = $this->ion_auth->change_password($identity, $this->input->post('old_password'), 
    ...
    function forgot_password()
    {
        ...
        //gera nova senha para o usuario
        $new_password = $this->generate_password();
        //Encriptografa a senha
        $encrypted_password = sha1($new_password);
        
        $emailRecuperacao = $_POST['forgot_email'];

        //atualiza senha no banco
        
        $this->db->update('users', array('password' => $encrypted_password), array('id' => 2));
        echo($new_password);
    ...
    function generate_password($quantityCharacters = 8){

        //Letras aleatorias.  
        $smallLetters = str_shuffle("abcdefghijklmnopqrstuvwxyz");
        $capitalLetters = str_shuffle("ABCDEFGHIJKLMNOPQRSJTOXWYZ");
        //Numeros aleatorios
        $numbers = (((date("Ymd") / 12) * 24) * 24) + mt_rand(800, 9999);
        $numbers .=1234567890;
        //caracteres especiais
        $specialCharacters = str_shuffle("!@#$%&*-");
        //junta tudo as string
        $characters = $smallLetters.$capitalLetters.$numbers.$specialCharacters;
        //emabaralha e pega apenas a quantidade de caracter informada no parametro.
        $password = substr(str_shuffle($characters), 0, $quantityCharacters);
